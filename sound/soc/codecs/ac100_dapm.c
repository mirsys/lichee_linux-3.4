...
static const struct snd_kcontrol_new ac100_controls[] = {
	...
	/*ADC*/
	SOC_SINGLE_TLV("LADC input gain", ADC_APC_CTRL, ADCLG, 0x7, 0, adc_input_vol_tlv),
	SOC_SINGLE_TLV("RADC input gain", ADC_APC_CTRL, ADCRG, 0x7, 0, adc_input_vol_tlv),
...
#ifdef CONFIG_SND_SUNXI_SOC_DAUDIO0_KARAOKE_MACHINE
	SND_SOC_DAPM_MUX("SPK_R Mux", SND_SOC_NOPM, 0, 0,	&ac100_rspks_func_controls),
	SND_SOC_DAPM_MUX("SPK_L Mux", SND_SOC_NOPM, 0, 0,	&ac100_lspks_func_controls),
#else
	...
	struct snd_soc_dai *codec_dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		...
		reg_val &= 0xf;
		if (codec_dai->playback_active && dmic_used && ((reg_val == 0x4)||(reg_val == 0x5))) {
	...
	*/
	...
	for (i = 0; i < ARRAY_SIZE(codec_aif1_fs); i++) {
		if (codec_aif1_fs[i].samplerate ==  params_rate(params)) {
			...
			} else if (codec_dai->capture_active && dmic_used && codec_aif1_fs[i].samplerate == 48000) {
				snd_soc_update_bits(codec, AIF_SR_CTRL, (0xf<<AIF1_FS), (0x5<<AIF1_FS));
			} else {
